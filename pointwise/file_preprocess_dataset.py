...
class ContrasDataset(Dataset):
    def __init__(self, filename, max_seq_length, tokenizer, aug_strategy=["sent_deletion", "term_deletion", "qd_reorder"], ratio=0.6, training_num=1.0):
    ...
    def anno_main(self, qd_pairs):
        ...
        for iidx, sent in enumerate(history):
            ...
            # segment_ids.extend([iidx % 2] * (len(sent) + 1))
        query_tok += ["[eos]"]
        # query_tok += ["[SEP]"]
        doc_tok += ["[eos]"]
        doc_tok += ["[SEP]"]
    ...
    def __getitem__(self, idx):
        ...
        aug_strategy = self._aug_strategy.copy()
        if len(qd_pairs) <= 2:
            if "qd_reorder" in aug_strategy:
                aug_strategy.remove("qd_reorder")
        ...
        while random_pos1 == random_pos2 and random_pos1 != -1:
            strategy2 = self._rnd.choice(aug_strategy)
            random_qd_pairs2 = qd_pairs.copy()
            random_qd_pairs2, random_pos2 = self.augmentation(random_qd_pairs2, strategy2)

...
s3_output_path = s3_rootdir + "/output/yutao/aol_6/"
...
def main():
    ...
    os.system(f"cd /cache/ss/Pretraining && CUDA_VISIBLE_DEVICES=0,1,2,3 python runBertContras.py --bert_model_path /cache/data/BertModel/ --per_gpu_batch_size 128 --log_path /cache/output/pretraining/logs/ --save_path /cache/output/pretraining/models/ --epochs 6 --temperature 0.1 --aug_strategy sent_deletion,term_deletion,qd_reorder --ratio 0.6")
    os.system(f"cd /cache/ss/Pointwise && python runBert.py --score_file_path /cache/output/BertContrastive.aol.6.10.128.sent_deletion.term_deletion.qd_reorder.score.txt --bert_model_path /cache/data/BertModel/ --per_gpu_batch_size 64 --log_path /cache/output/pointwise/logs/ --save_path /cache/output/pointwise/models/bert_sessionsearch.aol.6.10.128.sent_deletion.term_deletion.qd_reorder --epochs 3 --pretrain_model_path /cache/output/pretraining/models/BertContrastive.aol.6.10.128.sent_deletion.term_deletion.qd_reorder --learning_rate 5e-5")

...
class PairDataset(Dataset):
    ...
    def anno_main(self, qd_pairs):
        ...
        all_attention_mask = [1] * len(history_toks)
        query_tok += ["[eos]"]
        query_tok += ["[SEP]"]
        doc_tok += ["[eos]"]
        doc_tok += ["[SEP]"]
        ...
        all_attention_mask.extend([1] * len(query_tok))
        all_attention_mask.extend([1] * len(doc_tok))
    ...
    def __getitem__(self, idx):
        ...
        label = int(line[0])
        sample_a = line[1].split("\t")
        sample_b = line[2].split("\t")
        input_ids, attention_mask, segment_ids = self.anno_main(sample_a)
        input_ids2, attention_mask2, segment_ids2 = self.anno_main(sample_b)
        batch = {

...
def main():
	...
	for m in models:
		...
		os.system(f'cd /cache/anchors/finetune && python runBert.py \
			--is_training \
		...
		print(f"[msmarco rerank] [{m}] training and evaluating.........")
		os.system(f'cd /cache/anchors/finetune && python runBert.py \
			--is_training \
			--per_gpu_batch_size {bs_predevice_train} --per_gpu_test_batch_size {bs_predevice_test} --task msmarco \
			--bert_model /cache/mymodel/{m} \
			--dataset_script_dir /cache/anchors/data_scripts \
			--dataset_cache_dir /cache/negs_rerank_msmarco_cache_{m} \
			--log_path /cache/output/rerank/log_{m}.txt \
			--train_file /cache/train_r_data \
			--dev_file  /cache/dev_r_data/all.json \
			--dev_id_file /cache/dev_r_data/ids.tsv \
			--msmarco_score_file_path /cache/output/rerank/score_{m}.txt \
			--msmarco_dev_qrel_path /cache/msmarco/msmarco-docdev-qrels.tsv \
			--save_path /cache/output/rerank/{m}.bin \
			--epochs {epoch} \
			--id {m}_rerank_msmarco')	

		print(f"[trecdl rerank] [{m}] evaluating.........")
		os.system(f'cd /cache/anchors/finetune && python runBert.py \
			--per_gpu_batch_size {bs_predevice_train} --per_gpu_test_batch_size {bs_predevice_test} --task trecdl \
			--bert_model /cache/mymodel/{m} \
			--dataset_script_dir /cache/anchors/data_scripts \
			--dataset_cache_dir /cache/negs_rerank_trecdl_cache_{m} \
			--log_path /cache/output/rerank/log_{m}_predict.txt \
			--train_file /cache/train_data \
			--dev_file  /cache/dev_data_trecdl_r/all.json \
			--dev_id_file /cache/dev_data_trecdl_r/ids.tsv \
			--msmarco_score_file_path /cache/output/rerank/score_{m}.txt \
			--msmarco_dev_qrel_path /cache/msmarco/trec-2019qrels-docs.txt \
			--save_path /cache/output/rerank/{m}.bin \
			--epochs {epoch} \
